...
<script>
	...
	export default {
		...
		computed: {
			getData() {
				console.log(this.$store.state.third.data, 111)
				return this.$store.state.third.data;
			}
		},
		mounted() {
			//window.TencentSports.supportApp.setShare(app.dataShare);
			var that = this
			var data = app.getsessionStorage('thirdData');
			data[0].selectGroup.message = 'A3';
			data[1].selectGroup.message = 'B3';
			data[2].selectGroup.message = 'C3';
			data[3].selectGroup.message = 'D3';
			data[4].selectGroup.message = 'E3';
			data[5].selectGroup.message = 'F3';
			data[6].selectGroup.message = 'G3';
			data[7].selectGroup.message = 'H3';
			...
			data = [{
					...
					'right': data[2].selectGroup
				},
				{
					'left': data[4].selectGroup,
					'right': data[6].selectGroup
				},
				{
					'left': data[1].selectGroup,
					'right': data[3].selectGroup
				},
				{
					'left': data[5].selectGroup,
					'right': data[7].selectGroup
				}
			]

			console.log(data, 333)
			this.copyData = data;
			this.$store.dispatch('getThird', {
				data
			});
//			bizBoss.report({
...
//				module: '4',
//				target: 'whole',
//				useraction: 'baoguang4'
//			})
		},
		methods: {
			nextStep() {
				if(this.$store.getters.r3.every(item => item.selectGroup.isSelected === true)) {
					app.setsessionStorage('fourthData', this.$store.getters.r3);
					this.$router.push({
						path: '/fourth'
					});
				} else {
